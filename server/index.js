import express from "express";
import bcrypt from "bcryptjs";
import jwt from "jsonwebtoken";
import sqlite3 from "sqlite3";
import { promisify } from "util";
import dotenv from "dotenv";

dotenv.config();

const JWT_SECRET = process.env.JWT_SECRET || "supersecret";
const PORT = process.env.PORT || 4000;

// init db with sqlite3 (async)
sqlite3.verbose();
const db = new sqlite3.Database("data.db");

const run = promisify(db.run.bind(db));
const get = promisify(db.get.bind(db));

(async () => {
  await run(`CREATE TABLE IF NOT EXISTS users (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    username TEXT UNIQUE NOT NULL,
    pass_hash TEXT NOT NULL)`);

  await run(`CREATE TABLE IF NOT EXISTS puzzle_progress (
    user_id INTEGER NOT NULL,
    puzzle_id TEXT NOT NULL,
    solved INTEGER NOT NULL DEFAULT 0,
    PRIMARY KEY (user_id, puzzle_id),
    FOREIGN KEY (user_id) REFERENCES users(id))`);
})();

const app = express();
app.use(express.json());

function authMiddleware(req, res, next) {
  const auth = req.headers.authorization;
  if (!auth?.startsWith("Bearer ")) return res.sendStatus(401);
  try {
    const payload = jwt.verify(auth.slice(7), JWT_SECRET);
    req.user = payload;
    next();
  } catch {
    res.sendStatus(401);
  }
}

app.post("/signup", async (req, res) => {
  const { username, password } = req.body;
  if (!username || !password) return res.status(400).json({ error: "Missing fields" });
  try {
    const hash = await bcrypt.hash(password, 10);
    await run("INSERT INTO users (username, pass_hash) VALUES (?, ?)", [username, hash]);
    const token = jwt.sign({ username }, JWT_SECRET, { expiresIn: "7d" });
    res.status(201).json({ token });
  } catch (e) {
    if (e.code === "SQLITE_CONSTRAINT_UNIQUE") {
      res.status(409).json({ error: "Username exists" });
    } else res.status(500).json({ error: "Server" });
  }
});

app.post("/login", async (req, res) => {
  const { username, password } = req.body;
  if (!username || !password) return res.status(400).json({ error: "Missing fields" });
  const row = await get("SELECT pass_hash FROM users WHERE username=?", [username]);
  if (!row) return res.status(401).json({ error: "Invalid" });
  const ok = await bcrypt.compare(password, row.pass_hash);
  if (!ok) return res.status(401).json({ error: "Invalid" });
  const token = jwt.sign({ username }, JWT_SECRET, { expiresIn: "7d" });
  res.json({ token });
});

app.get("/me", authMiddleware, (req, res) => {
  res.json({ username: req.user.username });
});

app.post("/progress", authMiddleware, async (req, res) => {
  const { puzzleId, solved } = req.body;
  if (!puzzleId) return res.status(400).json({ error: "Missing puzzleId" });
  const userRow = await get("SELECT id FROM users WHERE username=?", [req.user.username]);
  await run(
    "INSERT INTO puzzle_progress (user_id, puzzle_id, solved) VALUES (?, ?, ?) ON CONFLICT(user_id, puzzle_id) DO UPDATE SET solved=excluded.solved",
    [userRow.id, puzzleId, solved ? 1 : 0]
  );
  res.json({ ok: true });
});

app.listen(PORT, () => console.log(`API listening on :${PORT}`)); 
